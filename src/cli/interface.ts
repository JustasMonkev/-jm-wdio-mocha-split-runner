...
import { HookError, getRunnerName } from './utils.js'

const log = logger('@jm/wdio-mocha-split-runner:cli')
...
export default class WDIOCLInterface extends EventEmitter {
    ...
    constructor(
        ...
        super()
        this.hasAnsiSupport = Boolean(supportsColor && supportsColor.hasBasic)
    ...
    getFilenames(specs: string[] = []) {
        if (specs.length > 0) {
            const formattedSpecs = specs
                .map((spec) => spec.startsWith('file://') ? fileURLToPath(spec) : spec)
                .join(', ')
    ...
    printSummary() {
        ...
        return this.log(
            '\nSpec Files:\t',
            `${chalk.green(this.result.passed)} passed, ${retries}${failed}${skipped}${totalJobs} total`,
